#!/usr/bin/env bash
# TRIAGE: D255 md1400-capacity-monitor-lock
# Report/enforce governance contract coverage for MD1400 capacity visibility and guard policy.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --policy)
      ...
      ;;
    -h|--help)
      echo "Usage: d255-md1400-capacity-monitor-lock.sh [--policy report|enforce]"
      exit 0
      ;;
    *)
      echo "D255 FAIL: unknown arg: $1" >&2
      exit 2
      ;;
  esac
done
...
[[ "$MODE" == "report" || "$MODE" == "enforce" ]] || { echo "D255 FAIL: invalid policy mode '$MODE'" >&2; exit 2; }
...
if [[ "$FINDINGS" -gt 0 ]]; then
  if [[ "$MODE" == "enforce" ]]; then
    echo "D255 FAIL: md1400 governance findings=$FINDINGS"
    exit 1
  fi
  echo "D255 REPORT: md1400 governance findings=$FINDINGS"
  exit 0
fi

echo "D255 PASS: MD1400 capacity governance lock"
exit 0
