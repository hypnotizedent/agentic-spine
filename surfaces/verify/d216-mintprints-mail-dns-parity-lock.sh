#!/usr/bin/env bash
# TRIAGE: Enforce example-shop.com Microsoft 365 mail DNS records unchanged (MX/SPF/DKIM/DMARC/autodiscover).
# D216: example-shop-mail-dns-parity-lock
set -euo pipefail
...
if [[ "$STATUS" == "FAIL" ]]; then
  echo "D216 FAIL: example-shop mail dns parity lock: $VIOLATION_COUNT violation(s) in $CHECK_COUNT check(s)" >&2
  exit 1
fi

echo "D216 PASS: example-shop mail dns parity lock valid (checks=$CHECK_COUNT, violations=0)"
