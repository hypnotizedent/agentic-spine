#!/usr/bin/env bash
# TRIAGE: Domain registrar parity: canonical domains must have registrar snapshot with lock, nameservers, and expiry fields.
...
# --- Check 1: Each canonical root exists in snapshot ---
checks=$((checks + 1))
while IFS= read -r root; do
  [[ -z "$root" ]] && continue
...
# --- Check 2: Each canonical root has registrar_lock ---
checks=$((checks + 1))
while IFS= read -r root; do
  [[ -z "$root" ]] && continue
...
# --- Check 3: Each canonical root has nameservers ---
checks=$((checks + 1))
while IFS= read -r root; do
  [[ -z "$root" ]] && continue
...
# --- Check 4: Each canonical root has expiry_date ---
checks=$((checks + 1))
while IFS= read -r root; do
  [[ -z "$root" ]] && continue
