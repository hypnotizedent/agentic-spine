...
# Allowed: docs/legacy/**, docs/governance/**, docs/extract/**, docs/brain/**, .archive/**
...
if rg -n -S "$FORBID_RE" docs \
  -g'!docs/core/**' -g'!docs/legacy/**' -g'!docs/governance/**' -g'!docs/extract/**' -g'!docs/brain/**' \
  >/dev/null 2>&1; then
  echo "D16 FAIL: forbidden ronny-ops markers found in docs outside docs/core + docs/legacy + docs/governance + docs/brain" >&2
  rg -n -S "$FORBID_RE" docs -g'!docs/core/**' -g'!docs/legacy/**' -g'!docs/governance/**' -g'!docs/extract/**' -g'!docs/brain/**' | head -80 >&2
  exit 1
fi
