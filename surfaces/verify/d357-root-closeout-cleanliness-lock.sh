...
if (( ${#dirty_paths[@]} == 0 )); then
  echo "D357 PASS: root closeout cleanliness lock (root repo clean: $target_repo)"
  exit 0
fi
...
if (( ${#violations[@]} > 0 )); then
  echo "D357 FAIL: root closeout cleanliness lock" >&2
...
echo "D357 PASS: root closeout cleanliness lock (allowlisted tracked dirt only: ${#dirty_paths[@]} path(s))"
