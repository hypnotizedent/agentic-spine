...
if [[ "$status" == "PASS" ]]; then
  echo "D333 PASS: service-lifecycle-publish-enforcement (public_services=$public_count, lifecycle_hooks=wired, binding_defaults=enabled, issues=0)"
else
  echo "D333 FAIL: service-lifecycle-publish-enforcement (public_services=$public_count, missing_cap=$missing_count, issues: $issues_text)" >&2
  exit 1
fi
