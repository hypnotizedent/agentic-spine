...
try:
    ...
    print(f"D297 FAIL: unable to parse gaps YAML: {exc}", file=sys.stderr)
    raise SystemExit(1)
...
if violations:
    for gap_id, gid, age_days, gate_streak in violations[:20]:
        print(
            f"D297 FAIL: {gap_id} is open for {age_days}d while gate {gid} has pass streak={gate_streak} (expected auto-close)",
            file=sys.stderr,
        )
...
print("D297 PASS: gap auto-close health checks passed")
PY
