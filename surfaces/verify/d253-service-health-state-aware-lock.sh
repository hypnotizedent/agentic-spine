#!/usr/bin/env bash
# TRIAGE: D253 service-health-state-aware-lock
# Report/enforce governance contract coverage for health probe expected-state semantics.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --policy)
      ...
      ;;
    -h|--help)
      echo "Usage: d253-service-health-state-aware-lock.sh [--policy report|enforce]"
      exit 0
      ;;
    *)
      echo "D253 FAIL: unknown arg: $1" >&2
      exit 2
      ;;
  esac
done

[[ -f "$CONTRACT" ]] || { echo "D253 FAIL: missing $CONTRACT" >&2; exit 1; }
[[ -f "$MAPPING" ]] || { echo "D253 FAIL: missing $MAPPING" >&2; exit 1; }
[[ -f "$GAPS" ]] || { echo "D253 FAIL: missing $GAPS" >&2; exit 1; }
[[ -f "$W51" ]] || { echo "D253 FAIL: missing $W51" >&2; exit 1; }
command -v yq >/dev/null 2>&1 || { echo "D253 FAIL: yq missing" >&2; exit 1; }
command -v rg >/dev/null 2>&1 || { echo "D253 FAIL: rg missing" >&2; exit 1; }

if [[ -z "$MODE" ]]; then
  MODE="$(yq -r '.mode.default_policy // "report"' "$CONTRACT" 2>/dev/null || echo report)"
fi
[[ "$MODE" == "report" || "$MODE" == "enforce" ]] || { echo "D253 FAIL: invalid policy mode '$MODE'" >&2; exit 2; }
...
rg -qi "Health probes failing for stopped containers|state-aware" "$W51" || finding "MEDIUM" "W51 evidence marker for state-aware probe finding not present"

if [[ "$FINDINGS" -gt 0 ]]; then
  if [[ "$MODE" == "enforce" ]]; then
    echo "D253 FAIL: state-aware governance findings=$FINDINGS"
    exit 1
  fi
  echo "D253 REPORT: state-aware governance findings=$FINDINGS"
  exit 0
fi

echo "D253 PASS: service health state-aware governance lock"
exit 0
