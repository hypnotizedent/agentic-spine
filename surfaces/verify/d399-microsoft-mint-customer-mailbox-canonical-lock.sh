#!/usr/bin/env bash
# TRIAGE: Mint customer mail must converge on team@ only; fail on standalone info@, legacy redirect rules, duplicate inbox copies in team@, or shadow-copy drift into no-reply@.
set -euo pipefail
...
TEAM_MAILBOX="$(yq e -r '.write_authority.mint_customer_service_mailbox // ""' "$PROVIDERS_CONTRACT")"
INFO_ALIAS="$(yq e -r '.write_authority.mint_customer_service_ingress_aliases[0] // ""' "$PROVIDERS_CONTRACT")"
RONNY_MAILBOX="$(yq e -r '.write_authority.mint_executive_safety_copy_mailboxes[0] // ""' "$PROVIDERS_CONTRACT")"
...
[[ -n "$TEAM_MAILBOX" ]] || fail "communications.providers.contract missing write_authority.mint_customer_service_mailbox"
[[ -n "$INFO_ALIAS" ]] || fail "communications.providers.contract missing write_authority.mint_customer_service_ingress_aliases[0]"
[[ -n "$RONNY_MAILBOX" ]] || fail "communications.providers.contract missing write_authority.mint_executive_safety_copy_mailboxes[0]"
...
exec "$SECRETS_EXEC" -- "$TOKEN_EXEC" python3 - "$TEAM_MAILBOX" "$INFO_ALIAS" "$RONNY_MAILBOX" "$TRANSACTIONAL_SENDER" "$RECENT_TOP" "$SHADOW_WINDOW_MINUTES" <<'PY'
...
INFO_ALIAS = sys.argv[2]
RONNY_MAILBOX = sys.argv[3]
SYSTEM_SENDER_MAILBOX = sys.argv[4]
RECENT_TOP = int(sys.argv[5])
SHADOW_WINDOW_MINUTES = int(sys.argv[6])
...
info_user = get_user(INFO_ALIAS)
info_settings = get_mailbox_settings(INFO_ALIAS) if info_user else None
if info_user:
    violations.append(
        f"{INFO_ALIAS} still resolves as a standalone mailbox object "
        f"(accountEnabled={info_user.get('accountEnabled')}, userPurpose={info_settings.get('userPurpose') if info_settings else 'unknown'})"
    )

info_rules = get_inbox_rules(INFO_ALIAS) if info_user else []
info_redirect_rules = find_redirect_rules(info_rules, TEAM_MAILBOX)
if info_redirect_rules:
    violations.append(
        f"{INFO_ALIAS} still has redirect/forward rules into {TEAM_MAILBOX}: {', '.join(info_redirect_rules)}"
    )
...
print(
    ...
    f"(team_shared=true info_standalone=false redirect_rules=0 recent_messages={len(team_messages)} duplicates=0 system_sender_shadow_copies={len(system_sender_shadow_copies)})"
)
PY
