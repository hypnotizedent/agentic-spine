...
QUOTE_PREPARE_BIN="ops/plugins/domains/mint/bin/quote-prepare"
QUOTE_PACKET_NORMALIZER="ops/plugins/domains/mint/lib/quote_packet_normalize.py"
QUOTE_SOURCE_BIN="ops/plugins/domains/mint/bin/quote-source"
QUOTE_PACKET_SOURCER="ops/plugins/domains/mint/lib/quote_packet_source.py"
QUOTE_PRICE_BIN="ops/plugins/domains/mint/bin/quote-price"
QUOTE_PACKET_PRICER="ops/plugins/domains/mint/lib/quote_packet_price.py"
QUOTE_RENDER_BIN="ops/plugins/domains/mint/bin/quote-render"
QUOTE_PACKET_RENDERER="ops/plugins/domains/mint/lib/quote_packet_render.py"
QUOTE_PROMOTE_BIN="ops/plugins/domains/mint/bin/quote-promote"
QUOTE_PACKET_PROMOTER="ops/plugins/domains/mint/lib/quote_packet_promote.py"
QUOTE_PAYMENT_LINK_BIN="ops/plugins/domains/mint/bin/quote-generate-payment-link"
QUOTE_PACKET_PAYMENT_LINKER="ops/plugins/domains/mint/lib/quote_packet_payment_link.py"
QUOTE_SEND_BIN="ops/plugins/domains/mint/bin/quote-send"
QUOTE_PACKET_SENDER="ops/plugins/domains/mint/lib/quote_packet_send.py"
QUOTE_RECONCILE_PAYMENT_BIN="ops/plugins/domains/mint/bin/quote-reconcile-payment"
QUOTE_PACKET_PAYMENT_RECONCILER="ops/plugins/domains/mint/lib/quote_packet_payment_reconcile.py"
...
require_fixed "mint.quote.reconcile_payment:" "$PAYMENT_BRIDGE" "Payment bridge must define mint.quote.reconcile_payment"
require_fixed "GET /api/orders/:orderId/payments" "$PAYMENT_BRIDGE" "Payment bridge must document order-payment history reconciliation"
...
require_fixed "payment_record_status" "$QUOTE_PACKET_AUTHORITY" "Quote packet authority must define payment record reconciliation fields"
require_fixed "reconciled_at" "$QUOTE_PACKET_AUTHORITY" "Quote packet authority must define payment reconciliation timestamps"
...
require_fixed "name: mint.quote.reconcile_payment" "$QUOTE_PACKET_AUTHORITY" "Quote packet authority must define mint.quote.reconcile_payment"
require_fixed "quote_packet state updated to paid only when the resulting order payment_state is paid" "$QUOTE_PACKET_AUTHORITY" "Quote packet authority must govern honest packet paid-state projection"
...
require_fixed "quote_packet_payment_reconcile.py" "$QUOTE_RECONCILE_PAYMENT_BIN" "quote-reconcile-payment must delegate to the governed payment reconciliation runtime"
...
require_fixed "mint.quote.reconcile_payment:" "$CAPABILITIES" "Capabilities must register mint.quote.reconcile_payment"
...
require_fixed "mint.quote.reconcile_payment:" "$CAPABILITY_MAP" "Capability map must register mint.quote.reconcile_payment"
...
require_fixed '| `mint.quote.reconcile_payment` | mutating |' "$MINT_AGENT_CONTRACT" "Mint agent must describe mint.quote.reconcile_payment as mutating"
...
require_fixed 'Reads payment module `GET /api/v1/payments/checkout-session/:sessionId` plus `GET /api/orders/:orderId/payments`' "$MINT_AGENT_CONTRACT" "Mint agent must document governed payment reconciliation reads"
require_fixed 'Refuses to invent canonical paid state when the provider shows paid but the payment record has not yet reached `succeeded`' "$MINT_AGENT_CONTRACT" "Mint agent must document honest payment reconciliation blocking"
