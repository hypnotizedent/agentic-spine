...
[[ -f "$TRACKED" ]] || { echo "FAIL: no tracked config at $TRACKED"; exit 1; }

# Check it's valid JSON
python3 -m json.tool "$TRACKED" >/dev/null 2>&1 || { echo "FAIL: tracked config is not valid JSON"; exit 1; }

# Check button count
BUTTONS=$(python3 -c "import json; print(len(json.load(open('$TRACKED')).get('buttons',[])))")
[[ "$BUTTONS" -gt 0 ]] || { echo "FAIL: no buttons in tracked config"; exit 1; }
...
if [[ -f "$RUNTIME" ]]; then
  ...
  if [[ "$TRACKED_SHA" != "$RUNTIME_SHA" ]]; then
    echo "WARN: tracked config (sha:$TRACKED_SHA) differs from runtime (sha:$RUNTIME_SHA)"
    echo "  Run host.streamdeck.snapshot to sync"
  fi
fi

echo "PASS ($BUTTONS buttons)"
