#!/usr/bin/env bash
# TRIAGE: D256 credential-spof-lock
# Report/enforce governance contract coverage for credential redundancy and break-glass policy.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --policy)
      ...
      ;;
    -h|--help)
      echo "Usage: d256-credential-spof-lock.sh [--policy report|enforce]"
      exit 0
      ;;
    *)
      echo "D256 FAIL: unknown arg: $1" >&2
      exit 2
      ;;
  esac
done

[[ -f "$CONTRACT" ]] || { echo "D256 FAIL: missing $CONTRACT" >&2; exit 1; }
[[ -f "$MAPPING" ]] || { echo "D256 FAIL: missing $MAPPING" >&2; exit 1; }
[[ -f "$GAPS" ]] || { echo "D256 FAIL: missing $GAPS" >&2; exit 1; }
[[ -f "$W51" ]] || { echo "D256 FAIL: missing $W51" >&2; exit 1; }
command -v yq >/dev/null 2>&1 || { echo "D256 FAIL: yq missing" >&2; exit 1; }
command -v rg >/dev/null 2>&1 || { echo "D256 FAIL: rg missing" >&2; exit 1; }

if [[ -z "$MODE" ]]; then
  MODE="$(yq -r '.mode.default_policy // "report"' "$CONTRACT" 2>/dev/null || echo report)"
fi
[[ "$MODE" == "report" || "$MODE" == "enforce" ]] || { echo "D256 FAIL: invalid policy mode '$MODE'" >&2; exit 2; }
...
rg -qi "Single point of failure \(Ronny credentials\)|credential" "$W51" || finding "MEDIUM" "W51 evidence marker for credential SPOF finding not present"

if [[ "$FINDINGS" -gt 0 ]]; then
  if [[ "$MODE" == "enforce" ]]; then
    echo "D256 FAIL: credential SPOF governance findings=$FINDINGS"
    exit 1
  fi
  echo "D256 REPORT: credential SPOF governance findings=$FINDINGS"
  exit 0
fi

echo "D256 PASS: credential SPOF governance lock"
exit 0
