...
# PACKET-1395 + PACKET-1398: Mint suppliers L3 witness pair seam.
# Spine surface kept minimal per the subtraction packet — D441 asserts
# only that the cap bindings + manifest parity + plugin scripts +
# status.sh seam exist. Mint format / parsing / line-rendering live
# in mint-modules (scripts/ops/suppliers-witness.py and
# scripts/ops/suppliers-attention-line.py); D441 does not assert
# their bodies (cross-repo product code is not spine's verify scope).
...
MINT_SUPPLIERS_LINE_BIN="$SPINE_CODE/ops/plugins/domains/mint/bin/mint-suppliers-attention-line"
[[ -x "$MINT_SUPPLIERS_WITNESS_BIN" ]] || fail "mint.suppliers.witness plugin script missing"
[[ -x "$MINT_SUPPLIERS_POLL_BIN" ]] || fail "mint.suppliers.poll plugin script missing"
[[ -x "$MINT_SUPPLIERS_LINE_BIN" ]] || fail "mint.suppliers.attention.line plugin script missing"
grep -q 'mint.suppliers.witness:' "$SPINE_CODE/ops/capabilities.yaml" || fail "capability registry missing mint.suppliers.witness"
grep -q 'mint.suppliers.poll:' "$SPINE_CODE/ops/capabilities.yaml" || fail "capability registry missing mint.suppliers.poll"
grep -q 'mint.suppliers.attention.line:' "$SPINE_CODE/ops/capabilities.yaml" || fail "capability registry missing mint.suppliers.attention.line"
grep -q 'mint-suppliers-witness' "$SPINE_CODE/ops/plugins/MANIFEST.yaml" || fail "plugin manifest missing mint-suppliers-witness script"
grep -q 'mint-suppliers-poll' "$SPINE_CODE/ops/plugins/MANIFEST.yaml" || fail "plugin manifest missing mint-suppliers-poll script"
grep -q 'mint-suppliers-attention-line' "$SPINE_CODE/ops/plugins/MANIFEST.yaml" || fail "plugin manifest missing mint-suppliers-attention-line script"
grep -q '_render_mint_suppliers_attention' "$STATUS_BIN" || fail "ops status brief must invoke the Mint suppliers seam"
grep -q '_render_full_mint_suppliers_attention' "$STATUS_BIN" || fail "ops status full mode must invoke the Mint suppliers seam"
grep -q 'mint.suppliers.attention.line' "$STATUS_BIN" || fail "ops status seam must invoke mint.suppliers.attention.line so format/label stays Mint-owned (PACKET-1398: no Mint format strings or row parsers in spine status.sh)"
grep -q 'OPS_STATUS_BRIEF_MINT_SUPPLIERS_DISABLE\|OPS_STATUS_FULL_MINT_SUPPLIERS_DISABLE' "$STATUS_BIN" || fail "ops status mint suppliers seam must expose at least one disable env"
# Subtraction enforcement: spine plugin must NOT carry Mint format
# parsing. The writer cap is a thin yaml-to-canonical shim; row
# regex / detail-body parsing knowledge lives in mint-modules.
if grep -qE 'ROW_RE|parse_suppliers_rows|^suppliers\\\.' "$MINT_SUPPLIERS_WITNESS_BIN"; then
    fail "mint.suppliers.witness must NOT contain Mint suppliers row format knowledge (regex / parser); that lives in mint-modules per PACKET-1398"
fi
...
"$MINT_SUPPLIERS_LINE_BIN" --self-check >/dev/null || fail "mint.suppliers.attention.line self-check failed"
