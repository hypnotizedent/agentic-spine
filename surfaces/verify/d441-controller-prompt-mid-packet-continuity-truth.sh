...
# PACKET-1395 — first L3 witness adoption: mint.suppliers.witness.
# Folded into D441 (no new D-gate, per surface-expansion discipline). The
# witness reads mint.runtime.proof stdout, filters suppliers.* rows, writes
# per-poll yaml under canonical state, and surfaces actionable/stale lines
# in ops status --brief and full. Witness-only by construction; product
# logic stays in mint-modules.
...
grep -q 'PRODUCER_CAP = "mint.runtime.proof"' "$MINT_SUPPLIERS_WITNESS_BIN" || fail "mint.suppliers.witness must source from existing mint.runtime.proof producer (no new health command, per witness-readback adoption runbook)"
