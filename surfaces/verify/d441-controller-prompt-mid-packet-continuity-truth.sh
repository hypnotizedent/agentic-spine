...
NARRATOR_MARKDOWN_RULES_OUT="$("$COMMIT_NARRATOR_BIN" --limit 1 --include-diff --evaluate-rules --format markdown --skip-input-readbacks)"
...
[[ "$NARRATOR_MARKDOWN_RULES_OUT" == *"reasoning ("* ]] || fail "commit.narrator.status --format markdown --evaluate-rules must surface a reasoning line citing the rule_id when at least one rule matched"
