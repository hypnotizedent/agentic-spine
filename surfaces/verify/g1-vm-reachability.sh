...
while IFS=$'\t' read -r hostname ssh_target ssh_user; do
  ...
  ssh_cmd="$(ssh_build_cmd_with_fallback "$ssh_target" 5 2>/dev/null || true)"
  proxy_target=""

  if [[ "$ssh_cmd" == *"ProxyJump="* ]]; then
    proxy_target="$(ssh_resolve_probe_via "$ssh_target")"
    resolved_host="$(ssh_resolve_host "$ssh_target")"
    path_used="proxyjump"
  fi

  if [[ -z "$resolved_host" || ( "$path_used" == "unreachable" && -z "$proxy_target" ) || -z "$ssh_cmd" ]]; then
  ...
  read -r -a ssh_cmd_parts <<<"$ssh_cmd"
  set +e
  "${ssh_cmd_parts[@]}" "true" </dev/null >/dev/null 2>&1
  rc=$?
  set -e

  if [[ "$rc" -eq 0 ]]; then
    if [[ -n "$proxy_target" ]]; then
      printf "%-22s %-10s %-12s %s\n" "$hostname" "PASS" "$path_used" "${ssh_user}@${resolved_host} via ${proxy_target}"
    else
      printf "%-22s %-10s %-12s %s\n" "$hostname" "PASS" "$path_used" "${ssh_user}@${resolved_host}"
    fi
  else
    if [[ -n "$proxy_target" ]]; then
      printf "%-22s %-10s %-12s %s\n" "$hostname" "FAIL" "$path_used" "${ssh_user}@${resolved_host} via ${proxy_target}"
    else
