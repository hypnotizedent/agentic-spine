#!/usr/bin/env bash
# TRIAGE: Enforce example-shop.com Resend + Stalwart mail DNS records unchanged (MX/SPF/DKIM/DMARC).
# D218: example-shop-co-mail-dns-parity-lock
set -euo pipefail
...
if [[ "$STATUS" == "FAIL" ]]; then
  echo "D218 FAIL: example-shop.com mail dns parity lock: $VIOLATION_COUNT violation(s) in $CHECK_COUNT check(s)" >&2
  exit 1
fi

echo "D218 PASS: example-shop.com mail dns parity lock valid (checks=$CHECK_COUNT, violations=0)"
