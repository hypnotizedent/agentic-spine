...
PROTECTED_RUNTIME_LANES=("ews-import" "nas-primary-rsync")
PROTECTED_BRANCH_REGEXES=("^main$" "^codex/cleanup-night-snapshot-.*$")
PROTECTED_WORKTREE_GLOBS=("$SPINE_STATE/loop-scopes/LOOP-MAIL-ARCHIVER-MICROSOFT-DEEP-IMPORT-20260226.scope.md" "docs/reference/audits/MD1400_*")
...
if command -v yq >/dev/null 2>&1 && [[ -f "$CONTRACT" ]]; then
  ...
  SNAPSHOT_ROOT="$(yq e -r '.snapshot_policy.output_root // ""' "$CONTRACT" 2>/dev/null || echo "${HOME}/.local/state/agentic-spine/closeout_backups")"
  [[ -n "$SNAPSHOT_ROOT" && "$SNAPSHOT_ROOT" != "null" ]] || SNAPSHOT_ROOT="${HOME}/.local/state/agentic-spine/closeout_backups"
fi
...
[[ "${#PROTECTED_RUNTIME_LANES[@]}" -gt 0 ]] || PROTECTED_RUNTIME_LANES=("ews-import" "nas-primary-rsync")
...
PROTECTED_TOKENS+=("ews" "nas-primary")
