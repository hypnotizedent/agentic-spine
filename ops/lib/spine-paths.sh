...
spine_canonical_worktree_root() {
  ...
  if command -v yq >/dev/null 2>&1 && [[ -f "$contract_path" ]]; then
    canonical_root="$(yq e -r '.policy.canonical_worktree_root // "/Users/ronnyworks/code/.runtime/spine/tmp/worktrees"' "$contract_path" 2>/dev/null || echo "$canonical_root")"
  fi
