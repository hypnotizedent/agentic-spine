...
STATUS="$ROOT/ops/plugins/infra/bin/content-family-decommission-readiness"
...
echo "content family decommission readiness tests"
echo "════════════════════════════════════════"
...
pass_out="$(python3 "$STATUS" --root "$tmpdir" --brief --strict 2>&1)"
assert_contains "$pass_out" "PASS issues=0" "status passes on consistent readiness projection"
...
games = data["families"]["games"]
games["service_authority"]["planned_service_planes"] = ["download-stack"]
games["decommission_dependencies"]["planned_service_planes"] = ["download-stack"]
path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False))
PY

set +e
fail_out="$(python3 "$STATUS" --root "$tmpdir" --brief --strict 2>&1)"
fail_rc=$?
set -e
if [[ "$fail_rc" -eq 1 ]]; then
  pass "status fails when planned family backdoors residual plane"
else
  fail "status fails when planned family backdoors residual plane (rc=$fail_rc)"
fi
