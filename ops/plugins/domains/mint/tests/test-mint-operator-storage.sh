...
assert_contains "$(cat "$REGISTRY")" "contract_required: false" "launchd registry marks mintfiles mount as non-critical browse posture"
assert_contains "$(cat "$REGISTRY")" "monitor: false" "launchd registry stops treating mintfiles mount as required scheduler health"
