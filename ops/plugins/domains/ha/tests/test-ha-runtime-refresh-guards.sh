...
assert_file_contains "$BASELINE_REFRESH" 'spine_runtime_activate_managed_worktree "$CONTROL_ROOT"' "baseline refresh activates runtime worktree"
...
assert_file_contains "$SYNC_AGENT" 'spine_runtime_activate_managed_worktree "$CONTROL_ROOT"' "sync agent activates runtime worktree"
assert_file_contains "$SYNC_AGENT" 'RUNTIME_ROOT="${SPINE_RUNTIME_ACTIVE_ROOT}"' "sync agent records runtime worktree root"
