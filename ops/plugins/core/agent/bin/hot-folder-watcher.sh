...
check_dependencies() {
    ...
    if [[ -z "${ZAI_API_KEY:-}" ]]; then
        local inf_agent="${SPINE}/ops/plugins/providers/bin/infisical-agent.sh"
        if [[ -x "$inf_agent" ]]; then
...
main() {
    case "${1:-}" in
        ...
        --help|-h)
            ...
            echo "  SPINE_INBOX        Override inbox path (default: \$SPINE_INBOX)"
            echo "  SPINE_OUTBOX       Override outbox path (default: \$SPINE_OUTBOX)"
            echo "  SPINE_STATE        Override state path (default: \$SPINE_STATE)"
