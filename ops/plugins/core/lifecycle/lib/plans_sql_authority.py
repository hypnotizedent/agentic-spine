...
def resolve_paths(root: Path) -> tuple[Path, Path, Path]:
    ...
    index_path = Path(os.environ.get("PLANS_INDEX_PATH", str(state_root / "plans/index.yaml"))).expanduser()
    plans_dir = Path(os.environ.get("PLANS_DIR_PATH", str(state_root / "plans"))).expanduser()
...
def render_placeholder_doc(plan: dict[str, Any]) -> str:
    ...
    branch_retention_state = str(plan.get("branch_retention_state") or "").strip()
    tracking_ref = str(plan.get("tracking_ref") or "").strip()
    pr_url = str(plan.get("pr_url") or "").strip()
    branch_block = ""
    if branch_ref or worktree_path or branch_retention_state or tracking_ref or pr_url:
        branch_lines = ["## Branch Memory", ""]
        if branch_ref:
            branch_lines.append(f"- branch_ref: `{branch_ref}`")
        if tracking_ref:
            branch_lines.append(f"- tracking_ref: `{tracking_ref}`")
        if branch_retention_state:
            branch_lines.append(f"- branch_retention_state: `{branch_retention_state}`")
        if worktree_path:
            branch_lines.append(f"- worktree_path: `{worktree_path}`")
        if pr_url:
            branch_lines.append(f"- pr_url: {pr_url}")
        branch_block = "\n" + "\n".join(branch_lines) + "\n"
    return (
        f"# {plan_id}\n\n"
        f"> Projection placeholder generated by `planning.plans.reconcile` on {today_text()}.\n"
        f"> Authority row lives in the runtime shared authority SQLite store (table: `plans`).\n\n"
        f"- status: `{status}`\n"
...
def require_scope_file(index_path: Path, loop_id: str, *, field_name: str) -> Path:
    """Validate that a loop scope file exists. Raises RuntimeError if not."""
    scope_file = index_path.parent.parent / "loop-scopes" / f"{loop_id}.scope.md"
    if not scope_file.exists():
        raise RuntimeError(
            f"{field_name} must resolve to a real scope file: {loop_id} ({scope_file})"
        )
    return scope_file


def validate_plan_loop_bindings(
    index_path: Path,
    plan: dict[str, Any],
    *,
    fields: tuple[str, ...] = ("source_loop_id", "target_loop_id", "depends_on_loop"),
) -> list[str]:
    """Check all loop-referencing fields on a plan resolve to real scope files."""
    plan_id = str(plan.get("plan_id") or "").strip() or "PLAN-UNKNOWN"
    errors: list[str] = []
    for field_name in fields:
        loop_id = str(plan.get(field_name) or "").strip()
        if not loop_id:
            continue
        scope_file = index_path.parent.parent / "loop-scopes" / f"{loop_id}.scope.md"
        if not scope_file.exists():
            errors.append(f"{plan_id}: {field_name} does not resolve to a real scope file: {loop_id} ({scope_file})")
    return errors
...
def project_to_surfaces(
    ...
    *,
    ...
    for plan in payload.get("plans", []):
        ...
        if not pid:
            continue
        rendered = render_placeholder_doc(plan)
        if pid in by_id:
            target = by_id[pid]
            text = target.read_text(encoding="utf-8")
            if "Projection placeholder generated by `planning.plans.reconcile`" in text and text != rendered:
...
def db_parity_snapshot(conn: sqlite3.Connection, index_path: Path, plans_dir: Path) -> dict[str, Any]:
    ...
    plan_by_id = {
        str(plan.get("plan_id") or "").strip(): plan
        for plan in expected_plans
        if isinstance(plan, dict) and str(plan.get("plan_id") or "").strip()
    }
    stale_placeholder_docs: list[str] = []
    for plan_id in sorted(plan_set):
        doc_path = plans_dir / f"{plan_id}.md"
        if not doc_path.exists():
            continue
        text = doc_path.read_text(encoding="utf-8")
        if "Projection placeholder generated by `planning.plans.reconcile`" in text and text != render_placeholder_doc(plan_by_id[plan_id]):
            stale_placeholder_docs.append(plan_id)

    idx_hash = sha256_text(dump_yaml(actual_index))
