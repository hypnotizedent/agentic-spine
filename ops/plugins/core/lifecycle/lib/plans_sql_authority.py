...
DEFAULT_DB_NAME = "shared_authority.db"
DEFAULT_INDEX_NAME = "index.yaml"
DEFAULT_PLANS_DIR_NAME = "plans"
...
def resolve_state_root(root: Path) -> Path:
    env_state = str(os.environ.get("SPINE_STATE") or "").strip()
    if env_state:
        return Path(env_state).expanduser()
    contract = load_yaml(root / "ops/bindings/mailroom.runtime.contract.yaml") or {}
    if isinstance(contract, dict):
        contract_state = str(contract.get("state_root") or "").strip()
        if contract_state:
            return Path(contract_state).expanduser()
    return root / "mailroom/state"


def resolve_paths(root: Path) -> tuple[Path, Path, Path]:
    state_root = resolve_state_root(root)
    plans_root = state_root / DEFAULT_PLANS_DIR_NAME
    db_path = Path(os.environ.get("PLANS_DB_PATH", str(state_root / DEFAULT_DB_NAME))).expanduser()
    index_path = Path(os.environ.get("PLANS_INDEX_PATH", str(plans_root / DEFAULT_INDEX_NAME))).expanduser()
    plans_dir = Path(os.environ.get("PLANS_DIR_PATH", str(plans_root))).expanduser()
...
def render_placeholder_doc(plan: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"> Authority row lives in `$SPINE_STATE/shared_authority.db` (table: `plans`).\n\n"
        f"- status: `{status}`\n"
