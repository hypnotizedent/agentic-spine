...
test_gaps_file_validation() {
  ...
  if "$ROOT/ops/plugins/core/loops/bin/gaps-file" --id GAP-OP-TEST-001 2>/dev/null; then
  ...
  # Invalid type
  if "$ROOT/ops/plugins/core/loops/bin/gaps-file" --id GAP-OP-TEST-001 --type invalid --severity low --description "test" --discovered-by "test" 2>/dev/null; then
  ...
  # Invalid severity
  if "$ROOT/ops/plugins/core/loops/bin/gaps-file" --id GAP-OP-TEST-001 --type stale-ssot --severity invalid --description "test" --discovered-by "test" 2>/dev/null; then
  ...
  if "$ROOT/ops/plugins/core/loops/bin/gaps-file" --id GAP-OP-135 --type stale-ssot --severity low --description "test" --discovered-by "test" 2>/dev/null; then
...
test_gaps_close_validation() {
  ...
  # Missing --status
  if "$ROOT/ops/plugins/core/loops/bin/gaps-close" --id GAP-OP-135 2>/dev/null; then
  ...
  # Invalid status
  if "$ROOT/ops/plugins/core/loops/bin/gaps-close" --id GAP-OP-135 --status invalid 2>/dev/null; then
  ...
  # Non-existent gap
  if "$ROOT/ops/plugins/core/loops/bin/gaps-close" --id GAP-OP-NONEXISTENT --status fixed 2>/dev/null; then
  ...
  if [[ -n "$high_or_critical" && "$high_or_critical" != "null" ]]; then
    if "$ROOT/ops/plugins/core/loops/bin/gaps-close" --id "$high_or_critical" --status fixed 2>/dev/null; then
    ...
    if "$ROOT/ops/plugins/core/loops/bin/gaps-close" --id "$high_or_critical" --status fixed --regression-lock-id "LEGACY-NO-REGRESSION-LOCK-PRE-20260301" 2>/dev/null; then
