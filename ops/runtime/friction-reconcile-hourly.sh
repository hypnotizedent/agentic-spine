...
echo "[friction-reconcile-hourly] start $(date -u +%Y-%m-%dT%H:%M:%SZ)"

spine_job_run \
  "friction-reconcile-hourly:friction.reconcile" \
  "$CAP_RUNNER" cap run friction.reconcile -- --loop-id LOOP-AGENT-FRICTION-AUTOPILOT-20260301 --json

echo "[friction-reconcile-hourly] done $(date -u +%Y-%m-%dT%H:%M:%SZ)"
