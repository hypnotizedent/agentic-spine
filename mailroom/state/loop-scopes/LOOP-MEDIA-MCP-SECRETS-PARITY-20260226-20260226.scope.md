...
- **D129** (schema conventions): Triggered by this loop's edits to `operational.gaps.yaml` — legacy-excepted fields `notes` and `discovered_at` flagged because file was modified. Transient: resolves once commit clears working tree delta.
