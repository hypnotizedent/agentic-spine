...
|------|------|-------------|-------|
| Shop | infra-core | 100.x.x.x | Docker container, port 8053, SSH user `ubuntu` |
| Home | pihole-home | 100.x.x.x | LXC 105, port 53, SSH user `root` |
...
- **Conditional forwarding:** Enable for local hostname resolution — point to your router IP (e.g. 10.0.x.x), local domain `lan` or `home.arpa`
