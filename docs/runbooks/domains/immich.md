...
## Scope
Primary recovery flow for domain `immich`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain immich --status failed`
2. `./bin/ops cap run verify.run -- domain immich`
