...
## Scope
Primary recovery flow for domain `surveillance`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain surveillance --status failed`
2. `./bin/ops cap run verify.run -- domain surveillance`
