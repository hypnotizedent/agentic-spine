...
## Scope
Primary recovery flow for domain `backup`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain backup --status failed`
2. `./bin/ops cap run verify.run -- domain backup`
