...
## Scope
Primary recovery flow for domain `workbench`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain workbench --status failed`
2. `./bin/ops cap run verify.run -- domain workbench`
