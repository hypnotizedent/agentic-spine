...
## Scope
Primary recovery flow for domain `mint`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain mint --status failed`
2. `./bin/ops cap run verify.run -- domain mint`
