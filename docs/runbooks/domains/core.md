...
## Scope
Primary recovery flow for domain `core`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain core --status failed`
2. `./bin/ops cap run verify.run -- domain core`
