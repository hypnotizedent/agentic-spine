...
## Scope
Primary recovery flow for domain `proxmox-network`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain proxmox-network --status failed`
2. `./bin/ops cap run verify.run -- domain proxmox-network`
