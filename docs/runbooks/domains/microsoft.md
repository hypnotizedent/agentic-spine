...
## Scope
Primary recovery flow for domain `microsoft`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain microsoft --status failed`
2. `./bin/ops cap run verify.run -- domain microsoft`
