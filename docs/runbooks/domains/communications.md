...
## Scope
Primary recovery flow for domain `communications`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain communications --status failed`
2. `./bin/ops cap run verify.run -- domain communications`
