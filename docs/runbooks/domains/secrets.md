...
## Scope
Primary recovery flow for domain `secrets`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain secrets --status failed`
2. `./bin/ops cap run verify.run -- domain secrets`
