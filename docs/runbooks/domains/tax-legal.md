...
## Scope
Primary recovery flow for domain `tax-legal`.

## Detect
1. `./bin/ops cap run spine.log.query -- --since-hours 24 --domain tax-legal --status failed`
2. `./bin/ops cap run verify.run -- domain tax-legal`
