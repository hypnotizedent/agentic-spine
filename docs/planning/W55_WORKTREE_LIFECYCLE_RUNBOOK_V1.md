...
- `./bin/ops cap run wave.closeout.finalize -- --receipt docs/planning/Wxx_..._RECEIPT.md --message "docs(<wave>): finalize closeout receipts"`
...
- `D274` receipt closeout completeness lock
...
4. Run `wave.closeout.finalize` to commit/push expected closeout receipts and enforce D274.
5. Run cleanup in three phases; do not skip `report-only` and `archive-only`.
6. Use `worktree.lifecycle.rehydrate` if a terminal path disappears but branch still exists.

## Safety Rules
- Never run delete without token.
- Never run delete before receipt finalization (`wave.closeout.finalize`) passes.
