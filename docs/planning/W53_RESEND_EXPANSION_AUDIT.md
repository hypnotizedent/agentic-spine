...
| ID | Name | Purpose |
|----|------|---------|
| D257 | resend-mcp-transactional-send-authority-lock | No Resend MCP send_email in governed paths |
| D258 | communications-resend-webhook-schema-lock | Webhook event schema contract present |
| D259 | communications-contacts-governance-lock | Contacts mutations require approval |
| D260 | communications-broadcast-governance-lock | Broadcast sends require approval + rate guard |
| D261 | n8n-resend-direct-bypass-lock | n8n must not call Resend API directly |
| D262 | communications-resend-expansion-contract-parity-lock | Contract + policy docs present and consistent |
