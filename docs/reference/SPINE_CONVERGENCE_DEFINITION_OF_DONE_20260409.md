...
- `$HOME/code/communications-domain`
- `$HOME/code/media-domain`
- `$WORKBENCH_ROOT/agents/<domain>`
...
Whether `$HOME/code/communications-domain`, `$HOME/code/media-domain`, or `$HOME/code/agentic-foundation` are later merged, archived, or narrowed is a **separate** architecture question. It is not required to call the spine program done.
...
- all criteria satisfied:
  ...
  - n8n extracted to `workbench/agents/n8n` (spine `704a7745`)
  - immich extracted to `workbench/agents/immich` (spine `704a7745`)
