...
1. Scaffold (dry-run):
```bash
./bin/ops cap run platform.inventory.intake -- scaffold --class part --id sas-cable-0gyk61 --owner-agent mint-agent --site shop --location-id shop-rack-bin-a1 --runbook-path docs/reference/generated/W43_INVENTORY_INTAKE_UNION_LOCK_20260223.md --dry-run
```
2. Validate envelope:
```bash
