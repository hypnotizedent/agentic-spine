...
- Generated: `2026-03-17T07:07:09Z`
...
| /mnt/media/downloads | runtime | regenerable | 2.2T | Active import/download lane for SABnzbd, qBittorrent, and related media tooling. |
...
| /mnt/media/movies-archive | runtime | payload | 205G | Live movies-archive root reachable over the canonical /media export. |
...
| media-stack-legacy-backups-drain | done | /media/backups | 0 | Canonical media config backups already live under /md1400/backup-cold/apps/media-config. These warm-lane tarballs are legacy archive residue and now live on md1400, not on media.
 |
| media-forensic-snapshot-hold | done | media@forensic-20260226-2325 | 0 | The retained forensic snapshot from the copy-first utilization upgrade was destroyed on 2026-03-17 after the duplicate-download hold and legacy media-stack archive residue were captured on md1400.
 |
| media-downloads-duplicate-drain | done | /mnt/media/downloads | 152G | Exact-name duplicate download directories and empty top-level residue were removed from the live downloads lane during the 2026-03-17 cleanup wave. Non-empty duplicates were parked under md1400 as a reviewable archive hold instead of being silently deleted.
 |
| media-downloads-reclaim | candidate | /mnt/media/downloads | 2.2T | Downloads remain the largest regenerable consumer on the warm lane. The storage-side duplicate drain and snapshot deletion freed meaningful runway, but the remaining backlog still needs tool-aware cleanup because download-stack automation continues to report zero-free-space imports.
 |
...
- `media`: media is at 95% usage. NFS-visible canonical payload already accounts for about 15.9TiB (movies/tv/music plus movies-archive), downloads still adds about 2.25TiB of regenerable pressure, the legacy warm-lane media-stack tarballs and duplicate-download hold now sit under md1400 archive lanes, and the forensic snapshot is gone. The remaining live blocker is that download-stack still sees only about 798G available over NFS and Sonarr continues logging zero-free-space imports. (ops/bindings/shop.media.pressure.authority.yaml)
