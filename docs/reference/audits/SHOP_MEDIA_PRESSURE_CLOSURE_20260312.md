...
- `nas-primary` is the cold lane with headroom: `7.30T used`, `36.3T free`, `16%` usage.
...
| `/mnt/media/backups` | compatibility hold | `103.5G` at observation time | Legacy warm-lane tarballs being drained to `nas-primary` |

## Local mount overlay drift

- `pve` local ZFS mountpoints show `media/movies`, `media/tv`, and `media/music` as empty child datasets.
- `nas-primary/media-cold/movies-archive` is mounted locally at `/media/movies-archive` and hides the host-local view there.
...
- Legacy `media-stack-2026-*.tar.gz` files were moved from `/media/backups` to `/nas-primary/media-cold/legacy-media-stack-backups`.
...
- Canonical media config backup truth already lives on `nas-primary` under `/nas-primary/backup-cold/apps/media-config` per `ops/bindings/backup.inventory.yaml`.
...
- The legacy warm-lane backup tarballs are now physically off the media pool and parked on `nas-primary`.
