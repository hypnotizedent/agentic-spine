...
For Mint customer work, the only authoritative new-work queue is `team@example-shop.com / Inbox`.

- `info@example-shop.com` is a protected historical mailbox and must remain readable without being repointed, merged, or mutated by agents.
- `ronny@example-shop.com` is executive/safety-copy only and not the normal customer-service queue.
- `no-reply@example-shop.com` is a hidden transactional sender only and must not receive shadow copies of human customer mail.
...
- Microsoft mailbox mutations must be receipt-backed and must not target `info@example-shop.com` or `info-legacy@example-shop.onmicrosoft.com`.

## Tenant Boringness

The Mint tenant should stay small on purpose.

- Active human Microsoft identities are `ronny@example-shop.com`, `mike@example-shop.com`, and `eva@example-shop.com`.
- `team@example-shop.com` remains the only canonical customer shared mailbox.
- `no-reply@example-shop.com` stays hidden and transactional-only.
