...
- Total governed capabilities with `domain: core`: `50`
- `48` are `plane: fabric` capabilities
- `2` are `plane: domain_external` capabilities listed in the catalog block below: `translator.ingest`, `translator.status`
- The catalog view shows only the `domain_external` capabilities; the `48` fabric capabilities are engine-internal
