...
| System | IP | Access |
|--------|-----|--------|
| proxmox-home | 100.x.x.x | `ssh root@proxmox-home` |
| NAS (Synology) | 100.x.x.x | https://nas:5001 |
| Home Assistant | 100.x.x.x | http://ha:8123 |
| Vaultwarden (decommissioned) | 100.x.x.x | Decommissioned 2026-02-16 (primary on infra-core) |
| Pi-hole | 100.x.x.x | http://pihole-home/admin |
...
| Interface | IP | Notes |
|-----------|-----|-------|
| Tailscale | 100.x.x.x | Primary access |
| Local | 10.0.x.x | Home LAN |
...
| Interface | IP | Notes |
|-----------|-----|-------|
| Tailscale | 100.x.x.x | `nas` hostname |
| Local | 10.0.x.x | Home LAN |
| DSM Web UI | https://10.0.x.x:5001 | - |
...
|------|----------|--------------|-----|------|--------|
| 100 | homeassistant | 100.x.x.x | 4GB | 32GB | Running |
| 101 | immich | — | — | — | **Destroyed** 2026-02-20 (shop VM 203 is canonical) |
| 102 | vaultwarden | 100.x.x.x | 2GB | 16GB | **Decommissioned** (2026-02-16; superseded by infra-core VM 204) |
...
| 105 | pihole-home | 100.x.x.x | **Running** | Home DNS + ad-blocking |
...
| Successor | Shop Immich (VM 203, 100.x.x.x) |
...
| Successor | Shop download-stack (VM 209, 100.x.x.x) |
...
## Home Network Summary (10.0.x.x/24)

### IP Address Map

| Device | Local IP | Tailscale IP | Role |
|--------|----------|--------------|------|
| Ubiquiti UDR | 10.0.x.x | - | Gateway, WiFi AP, DHCP |
| Synology NAS | 10.0.x.x | 100.x.x.x | Storage |
| proxmox-home | 10.0.x.x | 100.x.x.x | Hypervisor |
| SLZB-06 (Zigbee) | 10.0.x.x | - | Zigbee coordinator |
| SLZB-06MU (Matter) | 10.0.x.x | - | Zigbee + Matter |
| TubesZB (Z-Wave) | 10.0.x.x | - | Z-Wave coordinator |
| homeassistant | 10.0.x.x | 100.x.x.x | VM 100 |
| vaultwarden | 10.0.x.x | 100.x.x.x | VM 102 |
| media-home | 10.0.x.x | - | VM 106 |
| pihole-home | 10.0.x.x | 100.x.x.x | LXC 105 (active) |
| ~~download-home~~ | ~~10.0.x.x~~ | — | LXC 103 (destroyed 2026-02-20) |
...
|---------|-------|
| Subnet | 10.0.x.x/24 |
| Gateway | 10.0.x.x (UDR) |
| DHCP Range | 10.0.x.x - 10.0.x.x |
| DHCP DNS | 10.0.x.x primary (pihole-home), 10.0.x.x fallback |
| WiFi SSID | pogodobby |
...
|--------|-------|----|----------|--------|
| SLZB-06 | SMLIGHT SLZB-06 | 10.0.x.x | Zigbee (CC2652P) | Online |
| SLZB-06MU | SMLIGHT SLZB-06MU | 10.0.x.x | Zigbee + Matter | Online |
| TubesZB | TubesZB Z-Wave | 10.0.x.x | Z-Wave | Online |
...
**Email notifications:** P0 job sends to operator@example.com on all events.
...
| UDR DNS not using pihole | **RESOLVED** | ~~LOW~~ | DHCP DNS now set to 10.0.x.x primary with 10.0.x.x fallback. |
...
**Resolved 2026-02-07:**
- NAS SSH access: works as `ronadmin` via Tailscale (100.x.x.x)
...
- UDR DHCP DNS: 10.0.x.x (UDR itself)
...
| Hosts Verified | proxmox-home (100.x.x.x), NAS (100.x.x.x) |
